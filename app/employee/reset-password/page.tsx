import { verifyEmployeeResetToken } from "@/app/employee/actions";
...
export default async function ResetPasswordPage(props: {
  ...
  const { isValid } = await verifyEmployeeResetToken(token);

  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col md:flex-row select-none font-sans bg-white">
      
      {/* Left Column: Full height Slate Panel with Shadow Overlay */}
      <div className="hidden md:flex md:w-[40%] flex-col justify-between p-10 lg:p-12 relative overflow-hidden h-full bg-[#0f172a] z-10 shadow-[8px_0_30px_rgba(15,23,42,0.15)]">
        
        {/* Backdrop image mimicking workspace scene */}
        ...
        <div className="relative z-10 text-white flex flex-col gap-0.5">
          ...
          <span className="text-xs opacity-75 font-semibold">Employee Workspace Portal</span>
        </div>
        ...
        <div className="relative z-10 text-white my-auto max-w-xs">
          <h2 className="text-[28px] lg:text-[32px] font-extrabold tracking-tight leading-tight drop-shadow-sm">
            Secure your credentials and manage campaigns.
          </h2>
        </div>
        ...
        <div className="w-full max-w-[400px] mx-auto my-auto py-6">
          {!isValid ? (
            <div className="text-center py-8">
              ...
              <Link 
                href="/employee/login"
                className="inline-block bg-slate-900 hover:bg-slate-800 text-white font-extrabold py-3 px-6 rounded-full text-xs tracking-wider transition cursor-pointer"
              >
                Back to Sign In
              </Link>
            </div>

...
import { ChevronDown, SlidersHorizontal, Bookmark, X, Check, CheckCircle2, Briefcase, ShieldCheck } from "lucide-react";
import { useState, useRef, useEffect } from "react";
import { AllFiltersPanel } from "@/components/discover/all-filters-panel";
    ...
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-2 rounded-lg border px-3 py-1.5 text-[13px] font-medium transition ${
          isActive
            ? "border-indigo-400/80 bg-indigo-50/30 text-slate-700 hover:bg-indigo-50"
            ...
            {options.map((opt) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                  value === opt ? "bg-indigo-50 text-indigo-700 font-medium" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                }`}
              >
                {opt}
...
export function AdminTopFilters({
  filters,
  setFilters,
}: {
  filters: AdminFilterState;
  setFilters: (f: AdminFilterState) => void;
}) {
    ...
    <div className="flex flex-col border-b border-slate-200 bg-white relative z-30">
      ...
      <div className="flex h-16 flex-wrap items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4 border-l border-slate-200 pl-4">
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <button
              ...
              className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${
                filters.verified ? "bg-indigo-500" : "bg-slate-200"
              }`}
            >
              <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform shadow-sm ${filters.verified ? "translate-x-3.5" : "translate-x-0.5"}`} />
            </button>
            <span className="text-[12px] font-semibold text-slate-600 group-hover:text-slate-900 flex items-center gap-1">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <button
              onClick={() => setFilters({ ...filters, hasManager: !filters.hasManager })}
              className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${
                filters.hasManager ? "bg-indigo-500" : "bg-slate-200"
              }`}
            >
              <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform shadow-sm ${filters.hasManager ? "translate-x-3.5" : "translate-x-0.5"}`} />
            </button>
            <span className="text-[12px] font-semibold text-slate-600 group-hover:text-slate-900 flex items-center gap-1">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <button
              onClick={() => setFilters({ ...filters, brandSafe: !filters.brandSafe })}
              className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${
                filters.brandSafe ? "bg-indigo-500" : "bg-slate-200"
              }`}
            >
              <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform shadow-sm ${filters.brandSafe ? "translate-x-3.5" : "translate-x-0.5"}`} />
            </button>
            <span className="text-[12px] font-semibold text-slate-600 group-hover:text-slate-900 flex items-center gap-1">
      ...
      {filters.subCategories.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 bg-slate-50/50 px-6 py-3 border-b border-slate-100 shadow-inner">
          ...
          {filters.subCategories.map((subCat) => (
            <span
              key={subCat}
              className="inline-flex items-center gap-1 rounded-full border border-indigo-200 bg-indigo-50 py-1 pl-2.5 pr-1.5 text-[12px] font-medium text-indigo-700 shadow-sm"
            >
              {subCat}
              <button
